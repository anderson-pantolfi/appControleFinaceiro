document.addEventListener('DOMContentLoaded', () => {
    ...
    const atualizarGraficoCarteira = (dados) => {
        ...
        if (Object.keys(categorias).length === 0) {
            ...
            investmentChart.data.labels = Object.keys(categorias); investmentChart.data.datasets[0].data = Object.values(categorias); investmentChart.data.datasets[0].backgroundColor = ['#4f46e5', '#10b981', '#f59e0b', '#8b5cf6', '#ef4444'];
        }
    ...
    configForm('form-receita', 'receita', 'modal-receita'); configForm('form-despesa', 'despesa', 'modal-despesa'); configForm('form-investimento', 'investimento', 'modal-investimento');
    ...
    const renderizarCategorias = () => {
        ...
        popular('cat-receita', catsReceita); popular('cat-despesa', catsDespesa); popular('cat-invest', catsInvest);
    };
    ...
    document.getElementById('btn-add-cat-receita').onclick = () => abrirModalNovaCat('receita'); document.getElementById('btn-add-cat-despesa').onclick = () => abrirModalNovaCat('despesa'); document.getElementById('btn-add-cat-invest').onclick = () => abrirModalNovaCat('invest');

    document.getElementById('form-nova-categoria').addEventListener('submit', (e) => {
        e.preventDefault(); const novaCat = inputNomeNovaCat.value.trim(), tipo = inputTipoNovaCat.value;
        ...
        if (listas[tipo].some(cat => cat.toLowerCase() === novaCat.toLowerCase())) { document.getElementById('cat-error-msg').style.display = 'block'; return; }
        listas[tipo].push(novaCat); localStorage.setItem(storageKeys[tipo], JSON.stringify(listas[tipo])); renderizarCategorias(); modalNovaCat.style.display = 'none'; document.getElementById(`cat-${tipo}`).value = novaCat;
    });
    ...
    const abrirModalDeleteCat = (tipo) => {
        ...
        if (listas[tipo].length <= 1) { alert("Mantenha pelo menos uma categoria!"); return; }
        document.getElementById('delete-cat-name').textContent = catAtual; document.getElementById('delete-cat-type').value = tipo; modalConfirmDel.style.display = 'flex';
    };
    document.getElementById('btn-rm-cat-receita').onclick = () => abrirModalDeleteCat('receita'); document.getElementById('btn-rm-cat-despesa').onclick = () => abrirModalDeleteCat('despesa'); document.getElementById('btn-rm-cat-invest').onclick = () => abrirModalDeleteCat('invest');
    ...
    document.getElementById('btn-confirm-delete').addEventListener('click', () => {
        ...
        if(tipo === 'receita') { catsReceita = catsReceita.filter(c => c !== catNome); localStorage.setItem('catsReceita', JSON.stringify(catsReceita)); }
        if(tipo === 'despesa') { catsDespesa = catsDespesa.filter(c => c !== catNome); localStorage.setItem('catsDespesa', JSON.stringify(catsDespesa)); }
        if(tipo === 'invest') { catsInvest = catsInvest.filter(c => c !== catNome); localStorage.setItem('catsInvest', JSON.stringify(catsInvest)); }
        renderizarCategorias(); modalConfirmDel.style.display = 'none';
    });
    ...
    document.getElementById('btn-open-receita').onclick = () => mRec.style.display = 'flex'; document.getElementById('btn-open-despesa').onclick = () => mDesp.style.display = 'flex'; document.getElementById('btn-open-invest').onclick = () => mInv.style.display = 'flex';
    document.getElementById('close-receita').onclick = () => mRec.style.display = 'none'; document.getElementById('close-despesa').onclick = () => mDesp.style.display = 'none'; document.getElementById('close-invest').onclick = () => mInv.style.display = 'none';
    document.getElementById('close-nova-categoria').onclick = () => modalNovaCat.style.display = 'none'; document.getElementById('btn-cancel-delete').onclick = () => modalConfirmDel.style.display = 'none';
    window.onclick = (e) => { [mRec, mDesp, mInv, modalNovaCat, modalConfirmDel].forEach(m => { if(e.target === m) m.style.display = 'none'; }); };
    document.getElementById('btn-clear-data').onclick = () => { if(confirm("Apagar tudo?")) { localStorage.clear(); location.reload(); } };
